...
_MODULE_FILES = {
    ...
    "hedge_optimizer":  "6_hedge_optimizer.py",
    "granite_advisor":  "7_granite_advisor.py",
}
...
def run_pipeline(farm_input: dict) -> dict:
    ...
    try:
        ...
        for crop_entry in crops:
            ...
            try:
                mc = run_monte_carlo(
                    lat=lat, lon=lon, crop_type=name,
                    planting_window_start=pw_start, planting_window_end=pw_end,
    ...
    try:
        ...
        risk_result = run_risk_assessment(
            farm_input=farm_input, soil_anomaly=soil_anomaly,
            monte_carlo_all=monte_carlo_all, market_data=market_data,
    ...
    try:
        run_hedge_optimizer = _import("hedge_optimizer").run_hedge_optimizer
    ...
    try:
        run_granite_advisor = _import("granite_advisor").run_granite_advisor
        log.info("[7/7] granite_advisor (Granite Call 2)")
        advisory = run_granite_advisor(
            farm_input=farm_input, risk_result=risk_result,
            hedge_result=hedge_result, seasonal_outlook=seasonal_outlook,
        )
    except Exception as exc:
